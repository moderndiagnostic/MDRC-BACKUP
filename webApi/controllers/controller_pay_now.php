<?
class _pay_now extends controller{
	function init()
	{
		###
	}
	function onload()
	{
		$pay_name=mysqli_real_escape_string($this->app->set_db_conn(),$this->app->getPostVar('pay_name'));
		$pay_email=mysqli_real_escape_string($this->app->set_db_conn(),$this->app->getPostVar("pay_email"));
		$pay_phone=mysqli_real_escape_string($this->app->set_db_conn(),$this->app->getPostVar("pay_phone"));
		$pay_amount=mysqli_real_escape_string($this->app->set_db_conn(),$this->app->getPostVar("pay_amount"));
		$pay_message=mysqli_real_escape_string($this->app->set_db_conn(),$this->app->getPostVar("pay_message"));

		if($pay_name!='' && $pay_email!='' && $pay_phone!='' && $pay_amount!='' && $pay_message!='')
		{
			...
			if($diretPaymentOrderID!='')
			{
				...
				$result=["url"=>"https://www.mdrcindia.com/webApi/index.php?view=pay_now_redirect&id=$diretPaymentOrderID"];
