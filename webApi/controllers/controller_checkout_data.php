...
class _checkout_data extends controller
{
	...
	function onload()
	{
		...
		if ($userID != '' && $userPhone != '' && $deviceType != '' && $action == 'step1') {
			...
			$obj_model_tmp_cartmini->join_table("item_other_data", "left", array("item_department_ids", "description"), array("cart_item_id" => "item_id"));
			...
			if (count($rs_cartmini) > 0) {
				...
				foreach ($rs_cartmini as $item) {
					...
					// Basic values
					$itemID     = $item['item_id']; // or cart_item_id (both same)
					$item_price_id = $item['item_price_id'];
					$itemname   = $item['item_name'];
					$itemslug   = $item['item_slug'];
					$price      = (float)$item['item_price_price'];
					$mrp        = (float)$item['item_price_mrp'];
					$test_count = (int)$item['item_test_count'];
					...
					// Description
					$description_li = '';
					if (!empty($item['item_other_data_description'])) {
						$description = strip_tags($item['item_other_data_description']);
						$description = mysqli_real_escape_string($this->app->set_db_conn(), $description);
						$description = str_replace(["\r", "\n"], ' ', $description);
						$description = $this->app->utility->string_truncate($description, 150);
						$description_li = $description;
					}

					// Test parameters (OPTIONAL – currently blank as per your code)
					...
					$cartItems[] = [
						"itemID" => $this->app->utility->encrypt($itemID),
						...
						"itemInfo"   => $itemInfo,
						"line1"      => $test_count . ' Parameters Included',
						"line2"      => 'Reports in 12 hours'
					];

					$minAdvanceAmount += $item['item_min_price']>0 ? $item['item_min_price'] : $price;
				}
			}
			...
			if (count($rs_cartmini) > 0) {
				...
				for ($i = 0; $i < count($rs_cartmini); $i++) {
					...
					if ($rs_cartmini[$i]['item_other_data_item_department_ids'] == 2 && $step_third_heading == '') {
						...
						$labCollectionHeading = "Select Near By Lab/Collection Point";
					}
				...
				if ($sampleCollect == 'Yes' || $sampleCollect == '') {
				...
				$payments[] = ["paymentLabel" => "Pay Online / UPI / Cards / QR Code", "paymentValue" => "RAZORPAY"];
				// if($customer[0]['phone']=='9510069163')
				// {
				// 	$payments[] = ["paymentLabel" => "Pay Online / UPI / Cards / QR Code", "paymentValue" => "RAZORPAY"];
				// }
				// else
				...
				$couponEnterShow=$minAdvanceAmount>0?'No':'Yes';
				//ss
				$allTotals = ["wallet" => (int)$customer[0]['wallet'], "subtotal" => (int)$subtotal, "cartCount" => (int)$cartCount];
				$result = ["allTotals" => $allTotals, "homeCollectionStep" => $homeCollectionStep, "labCollectionStep" => $labCollectionStep, "payments" => $payments, "paymentHeading" => "Select Payment Method", "couponEnterShow" => $couponEnterShow, "cartItems" => $cartItems,"minAdvanceAmount"=>$minAdvanceAmount>0?(float)$minAdvanceAmount:''];
