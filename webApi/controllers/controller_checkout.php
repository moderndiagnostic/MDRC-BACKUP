...
class _checkout extends controller {
	...
	function onload()
	{
		...
		// 	"orderID"=>"2418",
		// 	"paymentID"=>"727",
		// 	"webViewUrl"=>SERVER_ROOT."/webApi/index.php?view=order_payment_webview&orderID=2418&paymentID=727"
		// ];
		...
		if($userID!='' && $userPhone!='' && $deviceType!='' && $grandTotal!='' && $walletUsed!='' && $paymentMode!='' && $sampleCollect!='')
		{
			...
			elseif($payment_method=='RAZORPAY')
			{
				...
				$result=[
					...
					"retryPayButton"=>"No",
					"cancelButton"=>"No",
					"webViewUrl"=>SERVER_ROOT."/webApi/index.php?view=order_payment_webview&orderID=".$order_id."&paymentID=".$orderPayID
				];
