...
class _career extends controller
{
	...
	function onload()
	{
		...
		$job_title = $this->app->getPostVar("job_title");
		
		$obj_model_job_opning=$this->app->load_model('job_opening');
		$job=$obj_model_job_opning->execute("SELECT",false,"","title='".$job_title."'");
		
		$job_opening_id =$job[0]['id'] ?? '';
