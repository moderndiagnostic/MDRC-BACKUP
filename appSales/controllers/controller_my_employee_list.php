<?
class _my_employee_list extends controller {
	function init() {
	}
	...
	function onload()
	{
		...
		if($employeeID!='' && $employeePhone!='' && $deviceType!='')
		{
			...
			foreach($employee as $item)
			{
				...
				$myEmployeeList[]=array("id"=>$id,"name"=>$item['name'],"image"=>$image,"designation"=>$item['master_designation_name'],"city"=>$item['city_name'],"mobile"=>$item['mobile']);
			}
			$result=["myEmployeeList"=>$myEmployeeList];
