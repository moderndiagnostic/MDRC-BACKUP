<?
class _hub_pickup_sample_detail extends controller {
	function init() {
	}
	...
	function onload()
	{
		...
		{
			$samplePickupIDCond=" and employee_sample_pickup.id='".$samplePickupID."'";
			...
			$sampleCollect=count($sample_pickup)<=0?'':$sample_pickup[0]['sample_count'];
			$paymentCollect=count($sample_pickup)<=0?'':$sample_pickup[0]['payment_amount'];
			...
			$result=["sampleCollect"=>$sampleCollect,"paymentCollect"=>$paymentCollect,"pickupDetail"=>$pickupDetail,"clientDetail"=>$clientDetail,"summary"=>$pickupButtons['summary'],"personList"=>$personAssigned];
