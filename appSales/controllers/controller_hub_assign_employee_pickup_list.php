<?
class _hub_assign_employee_pickup_list extends controller {
	function init() {
	}
	...
	function onload()
	{
		...
		if($employeeID!='' && $deviceType!='')
		{
			...
			$client = $obj_model_client->execute("SELECT",false,"","employee_sample_pickup.employee_id='".$selectedEmployeeID."' ".$whereCond."","employee_sample_pickup.id desc");
			$count=count($client);
			...
			$client = $obj_model_client->execute("SELECT",false,"","employee_sample_pickup.employee_id='".$selectedEmployeeID."' and hub_received='No' ".$whereCond."","employee_sample_pickup.id desc limit ".$start.",".$limit."");
