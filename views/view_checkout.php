	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-lg-4 right-cart">
				<?php 
					if(empty($_SESSION['payment_type']) || $_SESSION['payment_type'] == 'COD' || $_SESSION['min_price_amount'] == 0) 
					{ 
						$couponC='';
					}
					else
					{
						$couponC='display:none';
					}
						
					?>
				<div class="cart-extra-sevc div-for-data walletinfo" style="<?=$couponC;?>">
				...
				?>
				<div class="cart-extra-sevc div-for-data couponinfo" style="<?=$couponC;?>">
...
<script>
	...
	$(document).on("keyup change", "#min_price", function () {
		var default_min_price = parseFloat($('#default_min_price').val()) || 0;
		var total = parseFloat($('#total').val()) || 0;
		var min_price = parseFloat($(this).val()) || 0;
		var due_amount = (total - min_price).toFixed(2);
		if(min_price>total || default_min_price>min_price)
		{
			$('#min_price').val(default_min_price).trigger('change');
			return false;
		}
		$('#due_amount').html('<i class="fas fa-rupee-sign"></i> ' + due_amount);
	});
</script>