...
class _category extends controller {
	...
	function onload() {
		...
		$rs_banner=$obj_model_banner->execute("SELECT",false,"","status='Active' and banner_image!='' and (FIND_IN_SET ('category',banner.show_page) or FIND_IN_SET('".$searchKeword."', banner.show_page)) and (FIND_IN_SET ('".$_SESSION['cityID']."',banner.city_ids) or city_ids='')","sort_id ASC");
		$this->app->assign("rs_banner", $rs_banner);
			...
			$rs_faq_data = $obj_model_faq->execute("SELECT",false,"","faq_type='item_category' and status='Active' and faq_category_id='".$rs_cat[0]['id']."'","faq.sort_id asc");
