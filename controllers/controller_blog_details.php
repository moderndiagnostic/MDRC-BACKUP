...
class _blog_details extends controller{
	...
	function onload()
	{
		...
		if($records[0]['item_ids']!=''){
			...
			$items_records = $obj_model_all->execute("SELECT", false, "", "item.status='Active' and FIND_IN_SET (" . $_SESSION['cityID'] . ",item.city_ids) and item.id IN (" . $records[0]['item_ids'] . ") and item_price.city_id='" . $_SESSION['cityID'] . "'", "item.sort_order ASC", "");
			$this->app->assign("items_records",$items_records);
		} else {
			$this->app->assign("items_records","");
		}
	}
}
?>