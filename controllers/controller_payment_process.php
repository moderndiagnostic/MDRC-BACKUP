...
class _payment_process extends controller{
	...
	function onload()
	{
		...
		$razorpay_payment_id=$this->app->getPostVar('razorpay_payment_id');
		...
		else 
		{
			...
			{
				$gateway_order_status='Failed';
			}

			$obj_model_payment_data=$this->app->load_model("customer_order_payment_data");
			$result_pay=$obj_model_payment_data->execute("SELECT",false,"","payment_data='".$razorpay_order_id."'");
			...
			if(count($result_pay)>0) {
				...
				{ 
					...
					if(!empty($customerEmail))
					{
						...
						$send_data_array=['name'=>$customerName,'order_id'=>$orderID,'order_detail'=>$order_detail];
					...
					$this->app->redirect('payment-failed');
				}
			}
			else {
