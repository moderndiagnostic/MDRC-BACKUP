...
class _checkout extends controller{
	...
	function place_order()
	{
		...
		$min_price_amount=$_SESSION['min_price_amount'];
		$wallet_selected='No';
		$instruction='';
		$customerCond="customer_cart.customer_id='".$_SESSION['MDRCCustID']."'";
		$obj_model_tmp_cartmini = $this->app->load_model("customer_cart");
		...
		$customerEmail=$rs_customer[0]['email'];
		$customerName=$rs_customer[0]['name'];
		$subtotal=0;
		for($i=0;$i<count($rs_cartmini);$i++)
		{
		...
		if($payment_type=='COD' || $payment_type=='WALLET')
		{
			...
			if(!empty($customerEmail))
			{
				...
				$template_name='booking_place_user';
				$send_data_array=['name'=>$customer_name,'order_id'=>$orderID,'order_detail'=>$order_detail];
				$subject='Your test booking (#'.$order_id.') request has been successfully received.';
				$mail_for='Customer';
				$data=['template_name'=>$template_name,'send_data_arary'=>$send_data_array,'subject'=>$subject,'mail_for'=>$mail_for,'to_mail'=>$customerEmail];
				$this->app->utility->sendMial($data);
			}
		...
		elseif($payment_method=='ONLINE')
		{
			$pay_value=$min_price_amount>0?$min_price_amount:$pay_value;
			...
			$data_od["payment_mode"] = 'RAZORPAY';
			$data_od["payment_status"] = $payment_status;
			$data_od["payment_data"] = $razor_order_id;
			$data_od["transction_type"] = 'Order';
			$data_od["transction_date_time"] = date('d-m-Y H:i:s');
			$data_od["ip"] = $_SERVER['REMOTE_ADDR'];
			$data_od["entry_from"] = $deviceType;
			...
			$this->app->redirect("checkout_process/".$orderPayID);
		}
		else
		{
			$data_od = array();
