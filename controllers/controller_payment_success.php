	...
	function onload()
	{
		...
		if(count($rs_data)>0) {
			...
			if($orderPayID>0) {
				...
				$result_payment=$obj_model_payment_data->execute("SELECT",false,"","id='".$orderPayID."'");

				$obj_model_payment_data=$this->app->load_model("customer_order_payment_data");
				$result_pay=$obj_model_payment_data->execute("SELECT",false,"","order_master_id='".$orderID."' and payment_status='Success'");
				
				$this->app->assign("tracking_id",$result_payment[0]['transction_id']);
				$this->app->assign("pay_amount",$result_payment[0]['transaction_amount']);
				$this->app->assign("pay_status",$result_payment[0]['payment_status']);
				$this->app->assign("result_pay",$result_pay);
			}
			else {
