...
class _diseases extends controller {
	...
	function onload() {
		...
		//$citySlug=$explodeData[$total_datas-2];		
			...
			$rs_banner=$obj_model_banner->execute("SELECT",false,"","status='Active' and banner_image!='' and FIND_IN_SET ('diseases',banner.show_page) and (FIND_IN_SET ('".$_SESSION['cityID']."',banner.city_ids) or city_ids='')","sort_id ASC");
