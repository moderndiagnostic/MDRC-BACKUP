    ...
    var nextload=$('#results').find('.isload').val();
		if(nextload=='true' && loading==false) //there's more data to load
		{
					...
					$.post('../scripts/ajax/index.php',
						{'method':'packages','page': track_load,'type_ids':type_ids,'dieses_ids':dieses_ids,'total_data':total_data,'search_data':search_data,'category_ids':category_ids,'actionType':'list','sort_by':sort_by,'pageType':pageType,'city_id':city_id,'department_id':department_id,'site':'msite'},
							function(data)
							{
...
$('.categoryClick').click(function()
{
	if ($(this).hasClass('click')) {
		$('.categoryClick').removeClass('click');
		var check=false;
	} else {
		$('.categoryClick').removeClass('click');
		$(this).addClass('click');
	...
	var chk_vals=[];
	//$('input[name="category[]"]:checked').each(function() {chk_vals.push($(this).val());});
	$('.categoryClick.click').each(function() {chk_vals.push($(this).attr("data-value"));});
	if(chk_vals.length>0)
	{
		var time=chk_vals.join(',');
		$("#category_ids").val(time);
...
function get_filtered_data()
{
	...
	$.ajax({
         ...
         type:"POST",
         data:"method=packages&actionfunction=showData&page=1&type_ids="+type_ids+"&dieses_ids="+dieses_ids+"&total_data="+total_data+"&actionType=list&search_data="+search_data+"&category_ids="+category_ids+"&sort_by="+sort_by+"&city_id="+city_id+"&department_id="+department_id+"&pageType="+pageType+"&site=msite",
        cache: false,
