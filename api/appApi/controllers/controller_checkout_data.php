	...
	function onload()
	{
		...
		if($userID!='' && $userPhone!='' && $deviceType!='' && $action=='step1' && $sampleCollect!='')
		{
			...
			if(count($rs_cartmini)>0)
			{
				...
				if(count($customer_token)>0 && ($customer_token[0]['android_version']>1.7 || $customer_token[0]['ios_version']>2.0))
				{
				...
				$result=["allTotals"=>$allTotals,"homeCollectionStep"=>$homeCollectionStep,"labCollectionStep"=>$labCollectionStep,"payments"=>$payments,"paymentHeading"=>"Select Payment Method","couponEnterShow"=>$couponEnterShow,"minAdvanceAmount"=>$minAdvanceAmount>0?(string)$minAdvanceAmount:''];
