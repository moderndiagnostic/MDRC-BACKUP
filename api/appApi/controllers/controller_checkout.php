...
class _checkout extends controller {
	...
	function onload()
	{
		...
		$minTotal=mysqli_real_escape_string($this->app->set_db_conn(),$this->app->getPostVar("minTotal"));
		...
		if($userID!='' && $userPhone!='' && $deviceType!='' && $grandTotal!='' && $walletUsed!='' && $paymentMode!='' && $sampleCollect!='')
		{
			...
			if($labCollectionAddressID!='') {
				...
				if($labCollectionTime==''){
					$labCollectionTime=$homeCollectionTime;
					if($labCollectionTime==''){
						$message=array("message"=>"Please Select Time.","msgCode"=>"0");
						$opt=json_encode($message, JSON_UNESCAPED_UNICODE);
						echo $this->app->utility->indent($opt);
			...
			$customer_email=$rs_customer[0]['email'];
			...
			elseif($payment_method=='RAZORPAY')
			{
				...
				$data_od["order_master_id"] = $order_id;
				$data_od["customer_id"] = $customer_id;
				$data_od["transaction_amount"] =!empty($minTotal) && $minTotal>0 ? $minTotal : $pay_value;
				$data_od["payment_type"] = $payment_type;
				$data_od["payment_mode"] = $payment_method;
				...
				$amount = number_format(!empty($minTotal) && $minTotal>0 ? $minTotal : $pay_value, '2', '.', '');
				$orderData = [
				...
				$result=[
					...
					"msg1"=>"Thank You.",
					"msg2"=>"Your Booking(#".$order_id.") is Pending. Please complete the payment to confirm your booking.",
					"retryPayButton"=>"Yes",
					"cancelButton"=>"Yes",
					"webViewUrl"=>SERVER_ROOT."/appApi/index.php?view=order_payment_webview&orderID=".$order_id."&paymentID=".$orderPayID
				];
				$message=array("message"=>'success',"msgCode"=>"4","result"=>$result);
				$opt=json_encode($message, JSON_UNESCAPED_UNICODE);
				echo $this->app->utility->indent($opt);
				exit;
			}
