...
class _item_list extends controller
{
	...
	function onload()
	{
		...
		if($pageType=='Radiology' || $pageType=='Pathology')
		{
		...
		if ($cityID != '') {
			...
			if ($page == 0) {
				...
				if ($pageType == 'Popular Package') {
					...
					if(count($item_category) > 0) {
						$catSlugCond = " or FIND_IN_SET ('" . $item_category[0]['slug'] . "',banner.show_page) ";
					}
				...
				$rs_banner = $obj_model_banner->execute("SELECT", false, "", "status='Active' and (FIND_IN_SET ('" . $show_page . "',banner.show_page) " . $catSlugCond . ") and (FIND_IN_SET ('" . $cityID . "',banner.city_ids) or city_ids='')", "sort_id ASC");
			...
			$result = ["bannerList" => $bannerList, "itemList" => $itemList, "filterList" => $filterList, "title" => $title, "certificateImage" => IMAGE_SERVER_ROOT . '/uploads/pathology_image.png'];
