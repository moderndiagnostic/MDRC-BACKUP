...
class _cart extends controller{
	...
	function onload()
	{
		...
		else if($userID!='' && $userPhone!='' && $deviceType!='' && $action=='cartItemDelete' && $cartID!='')
		{
			...
			if(count($rs_cart)>0)
			{
				$depIDs=array_column($rs_cart,'cart_item_department_ids');
			...
			$result=["cartCount"=>$cartCount,"cartSubtotal"=>$cartLineTotal,"sampleCollectShow"=>$sampleCollectShow];
		...
		else if($userID!='' && $userPhone!='' && $deviceType!='' && $action=='cartItemPrescriptionAssign' && $cartID!='')
		{
			...
			$cartItemPrescriptionFile=IMAGE_SERVER_ROOT.'/uploads/prescription/'.$banner_img11;
			...
			if(count($cartItems)>0)
			{
				...
				foreach($cartItems as $item)
				{
					...
					$cartList[]=[
						...
						"cartItemPrescriptionFile"=>$item['prescription_data']!=''?IMAGE_SERVER_ROOT.'/uploads/prescription/'.$item['prescription_data']:'',
