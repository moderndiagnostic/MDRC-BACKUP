<!-- Start: Active order_dispatched_report List  -->
$(document).ready(function(){
	$('#table_order_dispatched_report').DataTable({
		"order": [[0, "desc" ]],
		"columnDefs": [ { 'targets': [4,5,10],"orderable": false } ],
  		'autoWidth':false,
		'processing': true,
		'serverSide': true,
		'serverMethod': 'post',
		'ajax': {
			'url':'scripts/ajax/index.php?method=order_dispatched_report&actionType=order_dispatched_report_list',
		},
...
<!-- End: Active order_dispatched_report List -->

function reset_data()
{
	$('#order_dispatched_report').hide();
	$('#counter_report').hide();
	...
	$.ajax({
		...
		data: "method=order_dispatched_report&actionType=SessionSet&search_start_date="+search_start_date+"&search_end_date="+search_end_date,
		success: function(data)
		{
			var oTable = $('#table_order_dispatched_report').dataTable( );
