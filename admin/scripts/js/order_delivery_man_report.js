<!-- Start: Active order_delivery_man_report List  -->
$(document).ready(function(){
	$('#table_order_delivery_man_report').DataTable({
		"order": [[0, "desc" ]],
		...
		'serverMethod': 'post',
		'ajax': {
			'url':'scripts/ajax/index.php?method=order_delivery_man_report&actionType=order_delivery_man_report_list',
		},
...
<!-- End: Active order_delivery_man_report List -->

function reset_data()
{
	$('#order_delivery_man_report').hide();
	$('#counter_report').hide();
	$('#search_delivery_boy').val('0');
	$('#search_start_date').val('');
	$('#search_end_date').val('');
	
	var search_delivery_boy='';
	...
	$.ajax({
		...
		data: "method=order_delivery_man_report&actionType=SessionSet&search_delivery_boy="+search_delivery_boy+"&search_start_date="+search_start_date+"&search_end_date="+search_end_date,
		success: function(data)
		{
			var oTable = $('#table_order_delivery_man_report').dataTable( );
...
$(function(){
	...
	var search_delivery_boy=$('#search_delivery_boy').val();
	...
	({
		...
		success: function(data)
		{
			$('#order_delivery_man_report').show();
			$('#counter_report').show();
			var oTable = $('#table_order_delivery_man_report').dataTable( );
			oTable.api().ajax.reload();		
		}
