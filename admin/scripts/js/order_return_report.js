<!-- Start: Active order_return_report List  -->
$(document).ready(function(){
	$('#table_order_return_report').DataTable({
		"order": [[0, "desc" ]],
		"columnDefs": [ { 'targets': [4,9],"orderable": false } ],
  		'autoWidth':false,
		'processing': true,
		'serverSide': true,
		'serverMethod': 'post',
		'ajax': {
			'url':'scripts/ajax/index.php?method=order_return_report&actionType=order_return_report_list',
		},
...
<!-- End: Active order_return_report List -->

function reset_data()
{
	$('#order_return_report').hide();
	$('#counter_report').hide();
	...
	$.ajax({
		...
		data: "method=order_return_report&actionType=SessionSet&search_delivery_boy="+search_delivery_boy+"&search_start_date="+search_start_date+"&search_end_date="+search_end_date,
		success: function(data)
		{
			var oTable = $('#table_order_return_report').dataTable( );
