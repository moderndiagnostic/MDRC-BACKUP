$(document).ready(function(){
	$('#table_state').DataTable({
		...
		'serverMethod': 'post',
		'ajax': {
			'url':'scripts/ajax/index.php?method=state&actionType=state_list',
		},
		'columns': [
		 	{ "data": "checkbox" },
		  	{ "data": "id" },
...
$(document).on("click",".state_addedit_onclick", function ()
{
	getId=$(this).data("id");
	$('#custom_ajax_preloader').show();
	$.ajax({
	type: 'POST',
	url: 'scripts/modal/index.php?method=state_addedit&id='+getId,
	dataType : 'html',
	data: $(this).serialize()
	})
	.done(function(data)
	{
		...
		$('#modal_state_addedit').modal('show');
...
$(document).on("click",".state_modal_submit", function ()
{
    $('#state_form').validate({
		rules:
		{
		...
		{
			$('.state_modal_submit').html('<span class="spinner-border spinner-border-sm mg-r-5" role="status" aria-hidden="true"></span> Loading...');
			$(".state_modal_submit").attr("disabled", true);
			var dataString = new FormData(form);
			dataString.append('method', 'state');
			dataString.append('actionType', 'stateAddEdit');
			$.ajax({
                dataType: 'json',
                type: "POST",
				...
				{
					$('.state_modal_submit').html('Submit');
					$(".state_modal_submit").attr("disabled", false);
				  ...
				  else  if(responseData.RESULT==0)
				  {
							...
							$('#modal_state_addedit').modal('hide');
							var oTable = $('#table_state').dataTable( );
...
$(document).on("click",".state_delete_onclick", function ()
{
            ...
            }, function (r)
			{
				if(r == true)
				{
					$.ajax(
					{
						  ...
						  dataType: 'json',
						  url: "scripts/ajax/index.php",
						  data: "method=state&actionType=stateDelete&getid="+getid,
						  success: function(responseData)
						  {
...
function mulitple_state_select()
{
			var chk_vals=[];
						...
						if(r == true)
						  {
							  ...
							  dataType: 'json',
							  url: "scripts/ajax/index.php",
							  data: "method=state&actionType=stateMultiDelete&ids="+ids,
							  success: function(responseData){
								  if(responseData.RESULT==0)
								  {
									  $.bootstrapGrowl('<h4><strong>Notification</strong></h4> <p>'+responseData.msg+'</p>', {type:'success',delay: 3000,allow_dismiss: true,offset: {from: 'top', amount: 20}});
