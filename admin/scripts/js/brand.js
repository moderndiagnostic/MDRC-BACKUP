...
$(document).ready(function(){
	$('#table_brand').DataTable({
		...
		'serverMethod': 'post',
		'ajax': {
			'url':'scripts/ajax/index.php?method=brand&actionType=brand_list',
		},
		'columns': [
		 	{ "data": "checkbox" },
		  	{ "data": "id" },
...
$(document).on("click",".brand_addedit_onclick", function ()
{
	getId=$(this).data("id");
	$('#custom_ajax_preloader').show();
	$.ajax({
	type: 'POST',
	url: 'scripts/modal/index.php?method=brand_addedit&id='+getId,
	dataType : 'html',
	data: $(this).serialize()
	})
	.done(function(data)
...
$(document).on("click",".brand_modal_submit", function ()
{
    $('#brand_form').validate({
		...
		submitHandler: function (form)
		{
			...
			dataString.append('method', 'brand');
			dataString.append('actionType', 'brandAddEdit');
			$.ajax({
                dataType: 'json',
                type: "POST",
				...
				{
					$('.brand_modal_submit').html('Submit');
					$(".brand_modal_submit").attr("disabled", false);
				  ...
				  else  if(responseData.RESULT==0)
				  {
							...
							$('#modal_brand_addedit').modal('hide');
							var oTable = $('#table_brand').dataTable( );
...
$(document).on("click",".brand_delete_onclick", function ()
{
            ...
            }, function (r)
			{
				if(r == true)
				{
					$.ajax(
					{
						  ...
						  dataType: 'json',
						  url: "scripts/ajax/index.php",
						  data: "method=brand&actionType=brandDelete&getid="+getid,
						  success: function(responseData)
						  {
...
function mulitple_brand_select()
{
			var chk_vals=[];
						...
						if(r == true)
						  {
							  ...
							  dataType: 'json',
							  url: "scripts/ajax/index.php",
							  data: "method=brand&actionType=brandMultiDelete&ids="+ids,
							  success: function(responseData){
								  if(responseData.RESULT==0)
								  {
									  $.bootstrapGrowl('<h4><strong>Notification</strong></h4> <p>'+responseData.msg+'</p>', {type:'success',delay: 3000,allow_dismiss: true,offset: {from: 'top', amount: 20}});
