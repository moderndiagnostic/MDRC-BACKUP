$(document).ready(function(){
	$('#table_payment_link').DataTable({
		...
		'serverMethod': 'post',
		'ajax': {
			'url':'scripts/ajax/index.php?method=payment_link&actionType=payment_link_list',
			"data": function(d) {
...
$(document).on("click",".payment_link_modal_submit", function ()
{
    $('#payment_link_form').validate({
		...
		submitHandler: function (form)
		{
			...
			dataString.append('method', 'payment_link');
            ...
            }, function (r)
			{
				if(r == true)
				{
					$.ajax(
					{
						  ...
						  dataType: 'json',
						  url: "scripts/ajax/index.php",
						  data: "method=payment_link&actionType=payment_linkDelete&getid="+getid,
						  success: function(responseData)
						  {
						...
						if(r == true)
						  {
							  ...
							  dataType: 'json',
							  url: "scripts/ajax/index.php",
							  data: "method=payment_link&actionType=payment_linkMultiDelete&ids="+ids,
							  success: function(responseData){
								  if(responseData.RESULT==0)
								  {
									  $.bootstrapGrowl('<h4><strong>Notification</strong></h4> <p>'+responseData.msg+'</p>', {type:'success',delay: 3000,allow_dismiss: true,offset: {from: 'top', amount: 20}});
