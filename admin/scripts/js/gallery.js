...
$(document).ready(function(){
	...
	$('#table_gallery').DataTable({
		...
		'ajax': {
			...
			'url':'scripts/ajax/index.php?method=gallery&actionType=gallery_list',
...
<!-- End: Active gallery List -->





<!-- Start: Active gallery addedit modal-->

$(document).on("click",".gallery_addedit_onclick", function ()

{
	...
	url: 'scripts/modal/index.php?method=gallery_addedit&id='+getId,
...

	{
		...
		$('#modal_gallery_addedit').modal('show');

		$('#custom_ajax_preloader').hide();

		

		$('#gallery_form').parsley();
...
<!-- End: Active gallery addedit modal -->



<!-- Start: modal gallery addedit submit  -->
...

{
    ...
    $('#gallery_form').validate({
...

		{
			...
			$('.gallery_modal_submit').html('<span class="spinner-border spinner-border-sm mg-r-5" role="status" aria-hidden="true"></span> Loading...');

			$(".gallery_modal_submit").attr("disabled", true);

			var dataString = new FormData(form);

			dataString.append('method', 'gallery');

			dataString.append('actionType', 'galleryAddEdit');
			...
			$.ajax({
...

				{
					...
					$('.gallery_modal_submit').html('Submit');

					$(".gallery_modal_submit").attr("disabled", false);
...

				  {
							...
							$('#modal_gallery_addedit').modal('hide');

							var oTable = $('#table_gallery').dataTable( );
...
<!-- End: modal gallery addedit submit -->
...

			{
...

				{
...

					{
						  ...
						  data: "method=gallery&actionType=galleryDelete&getid="+getid,
					...
					function (r){
						...
						if(r == true)

						  {
							  ...
							  data: "method=gallery&actionType=galleryMultiDelete&ids="+ids,
