...
if($get_actionType=="master_designation_list")
{
	$table_name='master_designation';
	## Read value
	...
	{
			$status='<img src="assets/img/status/'.$result[$i]['status'].'.png" onclick="javascript:change_status(\''.$result[$i]['id'].'\', \'master_designation\', \''.$result[$i]['status'].'\')" border="0" id="status_'.$result[$i]['id'].'" style="cursor:pointer" alt="'.$result[$i]['status'].'" data-toggle="tooltip" data-placement="right" title="Tooltip on right" title="'.$result[$i]['status'].'" />';
			$edit_btn='<button type="button" class="btn btn-xs btn-primary btn-icon master_designation_addedit_onclick" data-id="'.$result[$i]['id'].'"><i class="fas fa-edit"></i></button>';
			$delete_btn='<button type="button" class="btn btn-xs btn-danger btn-icon master_designation_delete_onclick" data-id="'.$result[$i]['id'].'" ><i class="fas fa-trash"></i></button>';
...
//Function for master_designation addedit
if($actionType=="master_designationAddEdit")
{
	...
	if($name!='' && $level!='')
	{
		...
		$obj_model_check = $app->load_model("master_designation");
		...
		$update_field = array();
		$update_field['level'] = $level;
		$update_field['name'] = $name;
		$update_field['status'] = $status;
		$obj_model_user = $app->load_model("master_designation");
...
if($actionType=="master_designationDelete")
{
	$getid=$app->getPostVar('getid');
	if($getid!= NULL && $getid>0)
	{
		$obj_change_city = $app->load_model('city');
		$rs_city= $obj_change_city->execute("SELECT",false,"","master_designation_id='".$getid."' and status!='Trash'");
		if(count($rs_city)==0)
		{
			$update_field = array();
			$update_field['status']= 'Trash';
			$obj_change_table = $app->load_model('master_designation');
		...
		{
			$msg='This master_designation City Exist.';
			$msgcode=1;
			echo $obj_json->encode(array("RESULT"=>$msgcode,"url"=>"","msg"=>$msg));
			exit;
		}
...
if($actionType=="master_designationMultiDelete")
{
