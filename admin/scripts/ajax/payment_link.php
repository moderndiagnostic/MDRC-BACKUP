...
if($get_actionType=="payment_link_list")
{
	...
	if($start_date!='')
	{
		$searchQuery.=" AND DATE(payment_links.created_at) BETWEEN STR_TO_DATE('".$start_date."', '%d-%m-%Y') AND STR_TO_DATE('".$end_date."', '%d-%m-%Y')";
	}
	if($employee_id!='' && $employee_id!=0)
	{
		$searchQuery.=" AND payment_links.employee_id='".$employee_id."'";
	}
	if($searchValue != '')
	{
		$searchQuery = " and (
	...
	{
		if($result[$i]['payment_link_transaction_status']=='Success')
		{
			$status='<span class="badge badge-success ml-1">Success</span>';
		}
		else if($result[$i]['payment_link_transaction_status']=='Fail')
		{
		...
		(
			"id"=>$result[$i]['id'],
			"employee_name"=>$result[$i]['employee_name'].'<br>'.$result[$i]['master_designation_name'].'<br>'.$result[$i]['lms_employee_code'],
			"amount"=>$app->utility->moneyFormatIndia($result[$i]['amount']),
			"client_name"=>$result[$i]['name'],
			"mobile"=>$result[$i]['mobile'],
			"payment_time"=>date('d-m-y H:i:s',strtotime($result[$i]['payment_link_transaction_created_at'])),
