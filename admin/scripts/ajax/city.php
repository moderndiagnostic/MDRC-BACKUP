...
if($get_actionType=="city_list")
{
	...
	$obj_table = $app->load_model($table_name);
	$result = $obj_table->execute("SELECT", false, "SELECT count(*) as allcount,".$table_name.".*,state.name AS state from ".$table_name." LEFT JOIN state AS state ON(state.id=".$table_name.".state_id)  where  ".$table_name.".status!='Trash' ".$searchQuery);
...
if($actionType=="cityAddEdit")
{
	...
	if($name!='' && $state_id!='')
	{
		...
		$rs_check=$obj_model_check->execute("SELECT",false,"","status!='Trash' and state_id='".$state_id."' and name='".$name."' ".$cond."");
		if(count($rs_check)>0)
		{
