...
//Function for active order_delivery_man_report datatbale loading
if($get_actionType=="order_delivery_man_report_list")
{
	...
	if($_SESSION['search_delivery_boy']!='' && $_SESSION['search_delivery_boy']!=0)
	{
		$con=" AND van_id='".$_SESSION['search_delivery_boy']."'";
	}
	else
	{
		$con="";
	...
	$search_end_date_cond=" AND STR_TO_DATE(`order_delivery_date`, '%d-%m-%Y') BETWEEN STR_TO_DATE('".$s_date."', '%d-%m-%Y') AND STR_TO_DATE('".$e_date."', '%d-%m-%Y')";				
	...
	$obj_model_order_master->join_table("user", "left", array("name","mobilephone","billing_city","email"), array("user_id"=>"id"));
	$obj_model_order_master->join_table("delivery_vans", "left", array("person_name","mobile"), array("van_id"=>"id"));
	$rs_orders=$obj_model_order_master->execute("SELECT",false,"","order_master.id!=0 ".$search_end_date_cond." AND (order_status='Delivered' or order_status='Return') ".$con."");
	...
	$obj_model_order_master->join_table("user", "left", array("name","mobilephone","billing_city","email"), array("user_id"=>"id"));
	$obj_model_order_master->join_table("delivery_vans", "left", array("person_name","mobile"), array("van_id"=>"id"));
	$result=$obj_model_order_master->execute("SELECT",false,"","order_master.id!=0 ".$search_end_date_cond." AND (order_status='Delivered' or order_status='Return') ".$con."");
	...
	$obj_model_order_master->join_table("user", "left", array("name","mobilephone","billing_city","email"), array("user_id"=>"id"));
	$obj_model_order_master->join_table("delivery_vans", "left", array("person_name","mobile"), array("van_id"=>"id"));
	$result=$obj_model_order_master->execute("SELECT",false,"","order_master.id!=0 ".$search_end_date_cond." AND (order_status='Delivered' or order_status='Return') ".$con." ".$searchQuery."");
	...
	$obj_model_order_master->join_table("user", "left", array("name","mobilephone","billing_city","email"), array("user_id"=>"id"));
	$obj_model_order_master->join_table("delivery_vans", "left", array("person_name","mobile"), array("van_id"=>"id"));
	$result=$obj_model_order_master->execute("SELECT",false,"","order_master.id!=0 ".$search_end_date_cond." AND (order_status='Delivered' or order_status='Return') ".$con." ".$searchQuery." ","order_master.id DESC limit ".$row.",".$rowperpage."");
