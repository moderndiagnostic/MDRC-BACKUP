...
//Function for active key_fetures datatbale loading

if($get_actionType=="item_key_fetures_list")

{

	$table_name='item_key_fetures';
...

	{
			...
			$status='<img src="assets/img/status/'.$result[$i]['status'].'.png" onclick="javascript:change_status(\''.$result[$i]['id'].'\', \'item_key_fetures\', \''.$result[$i]['status'].'\')" border="0" id="status_'.$result[$i]['id'].'" style="cursor:pointer" alt="'.$result[$i]['status'].'" data-toggle="tooltip" data-placement="right" title="Tooltip on right" title="'.$result[$i]['status'].'" />';

					

			$edit_btn='<button type="button" class="btn btn-xs btn-primary btn-icon item_key_fetures_addedit_onclick" data-id="'.$result[$i]['id'].'"><i class="fas fa-edit"></i></button>';	

			

			$delete_btn='<button type="button" class="btn btn-xs btn-danger btn-icon item_key_fetures_delete_onclick" data-id="'.$result[$i]['id'].'" ><i class="fas fa-trash"></i></button>';
...
if($actionType=="item_key_feturesAddEdit")

{
...

	{
...

		  {
			  ...
			  $obj_model_record = $app->load_model("item_key_fetures");
...
if($id!='')

		{
			...
			$slug=$app->utility->unique_slug('item_key_fetures','edit','slug',$app->getPostVar('name'),$id);
...

		{
			...
			$slug=$app->utility->unique_slug('item_key_fetures','add','slug',$app->getPostVar('name'),'');
...

		{
								...
								$new_name1=$pre_name.'.'.$file_info1->extension;
									...
									if($app->utility->upload_file($_FILES['file1']))
									{
										  ...
										  if($app->utility->store_uploaded_file($app->get_user_config("item_key_fetures"), $new_name1))
											{	
		...
		$obj_model_user = $app->load_model("item_key_fetures");
...
if($actionType=="item_key_feturesDelete")

{
...

	{
		...
		$obj_change_table = $app->load_model('item_key_fetures');

		$update_id = $obj_change_table->execute("UPDATE",false,"UPDATE item_key_fetures SET status='Trash' WHERE id='".$getid."'");
...
if($actionType=="item_key_feturesMultiDelete")

{
...

	{
...

		{
			...
			$update_id = $obj_change_table->execute("UPDATE",false,"UPDATE item_key_fetures SET status='Trash' WHERE id=".$temp_ids[$i]."");
