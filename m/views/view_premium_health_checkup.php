...
<!-- Main Start -->
<main class="health-package-page page-common mb-xxl">
	...
	<section class="banner-section ratio2_1 p-3">
		<div class="h-banner-slider">
			<?php for($i=0;$i<count($this->rs_banner);$i++){
				...
				$this->rs_banner[$i]['mobile_image']=$this->rs_banner[$i]['mobile_image']==''?$this->rs_banner[$i]['banner_image']:$this->rs_banner[$i]['mobile_image'];
			...
			<div>
				<div class="banner-box">
					<img src="<?=$image?>" alt="banner <?=$this->rs_banner[$i]['mobile_image'];?>" class="bg-img" />
				</div>
			</div>
			<?php } ?>
		</div>
