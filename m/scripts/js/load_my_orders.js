    ...
    var nextload=$('#results').find('.isload').val();
		...
		if(nextload=='true' && loading==false) //there's more data to load
		{
					...
					$.post('../scripts/ajax/index.php',
						{'method':'get_my_orders','page': track_load,'serach_keyword':serach_keyword,'total_products':total_products,'site':'msite'}, 
							function(data)
							{
...
function get_filtered_data()
{
	...
	var total_products=$("#total_products").val();
 	...
 	$.ajax({
         ...
         type:"POST",
         data:"method=get_my_orders&actionfunction=showData&page=1&serach_keyword="+serach_keyword+"&total_products="+total_products+'&site=msite',
        cache: false,
