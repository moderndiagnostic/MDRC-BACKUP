...
if($action_type=='cartItems')
{
	...
	if(count($rs_cartmini)>0)
	{
		...
		for($i=0;$i<count($rs_cartmini);$i++)
		{
			...
			$price_html='<p class="font-md fw-bold mb-0"><span>'.$app->utility->moneyFormatIndia($price).'</span></p>';
			if($mrp>0 && $mrp>$price)
			{
				$price_html.=' <p class=" font-base fw-bold ms-3 mb-0"><span class="font-sm cancle-rupee">'.$app->utility->moneyFormatIndia($mrp).'</span></p>';
			}
			$memberSatisfy='No';
			if($customer_members_id>0 && $rs_cartmini[$i]['customer_members_first_name']!='')
			{
				...
				$member_html='<div class="alert alert-primary alert-dismissible fade show p-2 alert-box">
					<a href="javascript:void(0)" type="button" data-id="'.$cartID.'" class="cartMemberRemove">
					<span class="d-block">'.$rs_cartmini[$i]['customer_members_prefix'].' '.$rs_cartmini[$i]['customer_members_first_name'].' '.$rs_cartmini[$i]['customer_members_last_name'].' | '.$rs_cartmini[$i]['customer_members_relation'].'</span>
					<span class="d-block">'.$line1.', '.$area.','.$city.' - '.$pincode.', '.$state.'</span>
					<i class="far float-end fa-times-circle" class="btn-close" type="button"></i>
					</a>
				</div>';
			...
			else
			{
				...
				{
					$extraItemsHtml='';
					$memberSelectClass='loginModal';
				}
				$member_html='<div class="border-top">
					<a class="d-block py-2 theme-color fw-bold '.$memberSelectClass.'" type="button" '.$extraItemsHtml.'>
						<i class="fas fa-plus" class="icon-size-1 theme-color me-1"></i>
						Add patients for this '.$item_type_name.'
					</a>
				</div>';
			}
			$prescription_html='';
			...
			if($prescription_require=='Yes')
			{
				...
				else
				{
					...
					{
						$extraItemsHtml='';
						$preSelectClass='loginModal';
					}
					$prescription_html='<div class="border-top">
						<a class="d-block py-2 theme-color fw-bold '.$preSelectClass.'" type="button" '.$extraItemsHtml.'>
							<i class="fas fa-plus" class="icon-size-1 theme-color me-1"></i>
							Add Prescription
						</a>
					</div>';
			...
			$other_html='<div class="">
							'.$member_html.' '.$prescription_html.'
						</div>';
						
			$html.='<div class="cart-list bg-white px-3 pt-3 pb-1 mb-3">
					<div class="d-flex justify-content-between">
						<a href="javascript:void(0);" class="font-md fw-bold itemsDetails" type="button" data-id="'.$cart_item_id.'">'.$itemName.' <i class="fas fa-chevron-down text-black"></i> </a>
						<a href="javascript:void(0);" class="text-danger cartDelete" data-id="'.$cartID.'" type="button">
							<img src="assets/images/mdrc/icons/trash.svg" alt="" class="me-1">
						</a>
					</div>
					<p class="btn-outline-label font-base">
						Includes: <span>'.$testCount.' test</span>
					</p>
		...
		if(count($rs_check_home_collection)>0)
		{
			...
			if(in_array($_SESSION['cityID'],$homeCollectionDisable)) {
				...
				$homeCollectionHtml='<div class="d-flex bg-white p-3 justify-content-between align-items-center mb-3">
					<label for="Service">
						<div class="d-flex justify-content-between align-items-center">
							<div class="w-25">
								<img src="assets/images/mdrc/my-cart/home-service.png" alt="" class="img-fluid w-100">
							</div>
							<div class="ms-3">
								<p class="mb-0 font-md fw-bold">Sample Home Collection Service.</p>
								<p class="mb-0">Blood tests can be done through home blood sample collection services.</p>
							</div>
						</div>
					</label>
					<div class="ms-3">
						<input type="checkbox" onclick="changeHomeCollection()" '.$disable.' id="home_collection" name="home_collection" value="Yes" '.$checked.'>
					</div>
				</div>';
		}
	}
