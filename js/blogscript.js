...
//detect page scroll
	$( "#load_more_blog" ).click(function(){
		...
		if(nextload=='true' && loading==false) //there's more data to load
		{
			...
			{'method':'get_blog','page': track_load,'cat':catv,'tag':tagv,'serach_keyword':serach_keyword,'total_blogs':total_blogs,'sort_by':sort_by},
			function(data){
...
function get_filtered_data()
{
	...
	$.ajax({
		...
		type:"POST",
		data:"method=get_blog&page=1&cat="+catv+"&tag="+tagv+"&serach_keyword="+serach_keyword+"&total_blogs="+total_blogs+"&sort_by="+sort_by,
		cache: false,
